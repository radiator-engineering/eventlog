...
`CHANGELOG.md` is generated by git-cliff from commit messages (`cliff.toml`).
Never edit it by hand, and never add changelog lines to a task; the commit
reactor's conventional commit message, written from your `result` summary, is
the changelog entry. Write `result` summaries as the user-facing line you want
to read there. At release the controller runs
`git cliff --tag vX.Y.Z -o CHANGELOG.md`, bumps `Cargo.toml`, tags, pushes,
and runs `cargo publish`; cargo-dist lifts that version's section into the
