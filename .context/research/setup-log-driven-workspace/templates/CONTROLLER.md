...
`.context/events.jsonl` is the append-only coordination log. Two reactors watch it, and nobody pings them: the **commit reactor** commits the files a `result` names; the **doc worker** then documents what was committed, editing only `{{DOC_PATHS}}`. Until a `result` is appended, nothing is committed and nothing is documented.
