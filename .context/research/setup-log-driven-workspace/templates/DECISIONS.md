...
## commit-agent = cursor-commit-reactor + {{COMMITTER_MODEL}} (autonomous)
...
**headless** `cursor-agent -p --force --trust --model {{COMMITTER_MODEL}}` to
...
- Editor churn files ({{CHURN}}) are untracked + ignored so they cannot keep
  the tree permanently dirty.

## doc-agent = doc-sync-reactor + headless Claude ({{DOC_MODEL}})
...
headless Claude (`claude -p --model {{DOC_MODEL}}`) loading three skills:
...
{{DOC_PATHS}}, never commits, then reports as
a **real worker** — `result by=doc-worker paths={{DOC_PATHS}}` — which the
