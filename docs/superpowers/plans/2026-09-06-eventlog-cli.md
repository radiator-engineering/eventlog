...
- [ ] **Step 1:** tests in a temp repo: `init` twice leaves identical files; `doctor` on a log whose seq 3 is `claim by=x agent=x` with no grant → `[FAIL] unsanctioned` (use a controller-only type: the built-in allowlist already permits `result` for any `by=` writer); after appending `decision key=log-writers value=x:claim` at seq 2 (rebuild the fixture) → OK. The rule is the as-of-seq allowlist only, nothing extra; `protect --status` exit 1 before, 0 after `protect` (skip on CI without permission; mark `#[ignore]` with a reason).
- [ ] **Steps 2–5.**
